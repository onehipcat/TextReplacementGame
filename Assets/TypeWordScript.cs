...
public class TypeWordScript : MonoBehaviour {
	...
	void Update () {
		if (active) {
			...
			if (textMesh.text == replacement) { //if the textmesh component we just changed doesn't match the target, then run this
	...
	void CheckLetter(char letter) { //creates variable for keycode passed to this function
		...
		if (index >= target.Length) {
			if (index < replacement.Length) {
				...
				StringBuilder sb = new StringBuilder(textMesh.text, replacement.Length); //creates the sb variable to refer to stringbuilder
